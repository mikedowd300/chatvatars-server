...
let client = require('socket.io').listen(process.env.PORT || 4300).sockets
let idWithUrl = {};
let usedAvatars = [];
...
client.on('connection', socket => {
  ...
  socket.on('disconnect', data => {
    avatars.push(idWithUrl[socket.id]);
    usedAvatars.splice(usedAvatars.indexOf(idWithUrl[socket.id]), 1);
    client.emit('display-avatars', { avatars, usedAvatars });
  });
